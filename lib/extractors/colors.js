  ...
  const result = await page.evaluate(() => {
    ...
    elements.forEach((el) => {
      ...
      if (isCta) {
        ...
        // CTA background is a strong primary signal — track separately
        const ctaNorm = normalizeColor(bgColor);
        if (ctaNorm && !ctaPrimaryMap.has(ctaNorm)) {
          ctaPrimaryMap.set(ctaNorm, bgColor);
        }
      }
    ...
    if (!semanticColors.primary && ctaPrimaryMap.size > 0) {
      // Pick the CTA color with highest score in colorMap
      let bestScore = -1;
      for (const [norm, orig] of ctaPrimaryMap) {
        const data = colorMap.get(norm);
        if (data && data.score > bestScore) {
          bestScore = data.score;
          semanticColors.primary = orig;
        }
      }
    }
