...
// DesignRefs design system (dark, Linear-inspired). This is the report's *chrome* —
// a DesignRefs report looks like DesignRefs. The extracted site's tokens are content
/*
 ...
 * the App's design system (designrefs-next/app/globals.css). color-scheme is
...
// DesignRefs brand mark (from designrefs-next/components/AppMarkIcon.tsx). Inlined,
...
/**
 * The top "ranking" row — DesignRefs's axes, not Lighthouse's. Drift (did it
...
export function generateHtmlReport(result: BrandingResult, options: HtmlReportOptions = {}): string {
  ...
  const version = options.version ?? result.meta?.designrefsVersion ?? "";
...
<meta name="generator" content="designrefs${version ? " " + esc(version) : ""}">
<title>DesignRefs report — ${esc(domain)}</title>
<style>${STYLE}</style>
</head>
<body>
...
${body}
<footer>Generated by <a href="https://github.com/hudsonargollo/designrefs">DesignRefs</a>${version ? " " + esc(version) : ""} · <a href="https://github.com/hudsonargollo/designrefs/issues">File an issue</a></footer>
</div>
</body>
</html>`;
}
